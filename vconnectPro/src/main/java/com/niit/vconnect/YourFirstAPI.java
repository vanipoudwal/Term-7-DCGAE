...
public class YourFirstAPI {
	...
	public Profile saveData(final @Named("name")String name,final @Named("email")String email,final @Named("profession")String profession,
		...
		final Profile p=new Profile(email,name,profession,about,cell);
		@Override
		public Profile run() {
			UserCounter u=(UserCounter)ConnectService.ofy().load().key(Key.create(UserCounter.class,"1")).now();
			if(u==null) {
	 ...
	 */
	@ApiMethod(name="logData", path="logData", httpMethod=HttpMethod.GET)
	public Profile logData(){
	Logger log=Logger.getLogger(Profile.class.getName());
	log.info("INFO log level is written to the log");
	return new Profile();
	}	

	@ApiMethod(name="readlogData" ,path="readlogData", httpMethod=HttpMethod.GET)
	public ReadLog readlogData(){
		...
		String result="<table><tr><th>Severity Level</th><th>Time</th><th>Message</th>";
		for(RequestLogs reqlog:logSvc.fetch(logquery)) {
			cal.setTimeInMillis(reqlog.getEndTimeUsec()/1000);
			for(AppLogLine applog:reqlog.getAppLogLines()) {
				result+="<tr><td>"+applog.getLogLevel()+"</td><td>"+applog.getTimeUsec()+"</td><td>"+applog.getLogMessage()+"</td></tr>";
			}
		}
		result+="</table>";
		ReadLog readlog=new ReadLog();
		readlog.setMessage(result);
		return readlog;
	}
}
